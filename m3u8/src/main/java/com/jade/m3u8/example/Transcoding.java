...
/**
 * @Date: 2022/10/01 10:24
 * @Description: javacpp官方模板
 */
...
public class Transcoding {
    ...
    static void openInput(String fileName) {
        ...
        for (int i = 0; i < inputFormatContext.nb_streams(); i++) {
            ...
            if (decoder == null)
                new RuntimeException("Unexpected decore: " + stream.codecpar().codec_id());
    ...
    static void initFilters() {
        ...
        for (int i = 0; i < inputFormatContext.nb_streams(); i++) {
            ...
            initFilter(filteringContexts[i], streamContexts[i].decoderContext, streamContexts[i].encoderContext,
                    filterSpec);
        }
    }
    ...
    static boolean encodeWriteFrame(AVFrame filterFrame, int streamIndex) {
        ...
        if (inputFormatContext.streams(streamIndex).codecpar().codec_type() == AVMEDIA_TYPE_VIDEO) {
            check(avcodec_encode_video2(streamContexts[streamIndex].encoderContext, encodedPacket, filterFrame,
                    gotFrameLocal));
        } else {
            check(avcodec_encode_audio2(streamContexts[streamIndex].encoderContext, encodedPacket, filterFrame,
                    gotFrameLocal));
        }
    ...
    static void flushEncoder(int streamIndex) {
        if ((streamContexts[streamIndex].encoderContext.codec().capabilities()
                    ...
                    if (filteringContexts[streamIndex].filterGraph != null) {
                        ...
                        try {
                            ...
                            if (type == AVMEDIA_TYPE_VIDEO) {
                                check(avcodec_decode_video2(streamContexts[streamIndex].decoderContext, frame, gotFrame,
                                        packet));
                            } else {
                                check(avcodec_decode_audio4(streamContexts[streamIndex].decoderContext, frame, gotFrame,
                                        packet));
                            }
                            if (gotFrame[0] != 0) {
